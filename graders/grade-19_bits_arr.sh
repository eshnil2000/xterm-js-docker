...
echo "Attempting to compile numToBits.c"
gcc -pedantic -Wall -Werror -std=gnu99  numToBits.c -o  numToBits 2>&1
if [ "$?" != "0" ]
then
...
timeout -s 9 5 ./numToBits 3</dev/null 4< /dev/null 5</dev/null 6</dev/null > temp.txt
...
clangStripFn "numToBits.c" "main" "int" > temp.c
cat temp.c /dev/fd/5 > temp2.c
gcc -pedantic -Wall -Werror -std=gnu99 temp2.c -o numToBits2 2>&1 
if [ "$?" != "0" ]
then
...
#array max and seed
    ...
    timeout -s 9 5 ./numToBits2 $x $y $y 3</dev/null 4< /dev/null 5</dev/null 6</dev/null > temp.txt
